...
.PHONY: build
build: $(BINDIR)/csi-driver $(BINDIR)/automount-runner
...
build-cross: $(GOX)
	CGO_ENABLED=0 $(GOX) -parallel=3 -output="_dist/{{.OS}}-{{.Arch}}/csi-driver_{{.OS}}_{{.Arch}}" -osarch='$(TARGETS)' $(GOFLAGS) -tags '$(TAGS)' -ldflags '$(LDFLAGS)' ./cmd/csi-driver
...
dist:
	( \
		...
		$(DIST_DIRS) tar -zcf eosxd-csi-${VERSION}-{}.tar.gz {} \; && \
		$(DIST_DIRS) zip -r eosxd-csi-${VERSION}-{}.zip {} \; \
	)
...
image: build-cross
	mkdir -p bin
	cp _dist/linux-amd64/csi-driver_linux_amd64 bin/csi-driver
	cp _dist/linux-amd64/automount-runner_linux_amd64 bin/automount-runner
	sudo $(IMAGE_BUILD_TOOL) build -t eosxd-csi:${DOCKER_TAG} -f deployments/docker/Dockerfile .
